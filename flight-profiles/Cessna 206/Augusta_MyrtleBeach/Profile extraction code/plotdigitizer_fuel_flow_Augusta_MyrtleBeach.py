...
# Load the image
image_path =  r"C:\Users\oadebajo\OneDrive\Documents\Flight load profiles\Augusta to Myrtle Beach\Profile10\Picture10_fuel.png" # Change this to your image file
...
np.savetxt(r"C:\Users\oadebajo\OneDrive\Documents\Flight load profiles\Augusta to Myrtle Beach\Profile10\fuel_flow\extracted_points.csv", data_points, delimiter=",", header="x_pixel,y_pixel", comments="")
...
transition_duration_start = 0.3  # Transition lasts 18s 
transition_duration_stop = 1  # Transition lasts 180s 
...
efficiency_points_time = [
    ...
    takeoff_start + transition_duration_start,  # End of first transition
    takeoff_stop - transition_duration_stop,  # Start of last transition
...
efficiency_points_values = [
    0.30,  # Before takeoff
    0.30,  # Start of takeoff
    0.19,  # After first 1-minute transition (takeoff)
    0.19,  # Before last 1-minute transition (takeoff)
    0.30,  # After takeoff stop (cruise)
    0.30  # After takeoff (cruise)
]
...
plt.plot(power_shaft_vs_time_fuel[:, 0], power_shaft_vs_time_fuel[:, 1], color='red', linewidth=0.5)
plt.title("Power vs time (kW)")
plt.show()

#Save interpolated time and power to shaft bsfc as a csv file
#np.savetxt(r"C:\Users\oadebajo\OneDrive\Documents\Flight load profiles\Columbia to Richmond\Profile 2\fuel_flow\interp_extracted_points_power2_shaft.csv", power_shaft2_vs_time_fuel, delimiter=",", header="Time_mins,power_kw", comments="")

#Save interpolated time and power to shaft as a csv file
np.savetxt(r"C:\Users\oadebajo\OneDrive\Documents\Flight load profiles\Augusta to Myrtle Beach\Profile10\fuel_flow\interp_extracted_points_power1_shaft.csv", power_shaft_vs_time_fuel, delimiter=",", header="Time_mins,power_kw", comments="")

#Save Power versus time as csv file
np.savetxt(r"C:\Users\oadebajo\OneDrive\Documents\Flight load profiles\Augusta to Myrtle Beach\Profile10\fuel_flow\interp_extracted_points_power_fuel.csv", power_vs_time_fuel, delimiter=",", header="Time_mins,Power_kW", comments="")

#Save Fuel flow vs. time file
np.savetxt(r"C:\Users\oadebajo\OneDrive\Documents\Flight load profiles\Augusta to Myrtle Beach\Profile10\fuel_flow\extracted_points_fuel_flow.csv", time_vs_fuel_flow, delimiter=",", header="Time_mins,fuel flow (Gal/hr)", comments="")
